...
ESX.ShowInventory = function()
	...
	for i=1, #ESX.PlayerData.accounts, 1 do
		if ESX.PlayerData.accounts[i].name == 'black_money' and ESX.PlayerData.accounts[i].money > 0 then
			local formattedMoney = _U('locale_currency', ESX.Math.Round(ESX.PlayerData.accounts[i].money))
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory',
	{
		...
		--[[if data.current.type == 'item_weapon' and data.current.ammo > 0 and player ~= -1 and distance <= 1.5 then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item',
		{
			...
			if data1.current.action == 'give' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to',
				{
					...
					local players, nearbyPlayer = ESX.Game.GetPlayersInArea(GetEntityCoords(playerPed), 1.5)
			...
			elseif data1.current.action == 'remove' then
				...
				if IsPedSittingInAnyVehicle(playerPed) then
					ESX.ShowNotification(_U('in_vehicle2'))
					return
				end
...
function giveanim()
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then
		...
		if ( IsEntityPlayingAnim( player, ad, anim, 8 ) ) then
			TaskPlayAnim( player, ad, "exit", 8.0, 3.0, 1750, 0, 1, 0, 0, 0 )
			ClearPedSecondaryTask(player)
		else
			SetCurrentPedWeapon( player, GetHashKey("WEAPON_UNARMED"), equipNow)
			Citizen.Wait(1)
			TaskPlayAnim( player, ad, anim, 8.0, 3.0, 1750, 0, 1, 0, 0, 0 )
		end
	end
end
