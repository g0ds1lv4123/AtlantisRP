...
AddEventHandler('esx:pickup', function(id, label, player)
	...
	TriggerServerEvent("esx:startTimeoutPickup", id)
end)

RegisterNetEvent('esx:removePickupTimeout')
AddEventHandler('esx:removePickupTimeout', function(id)
	Citizen.Wait(300000)
	if id ~= nil then 
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['F1']) and IsInputDisabled(0) and not isDead and not ESX.UI.Menu.IsOpen('default', 'es_extended', 'inventory') then
			ESX.ShowInventory()
		end
