...
AddEventHandler('esx_billing:sendBill', function(playerId, sharedAccountName, label, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', sharedAccountName, function(account)
		...
		elseif account == nil then
			...
			if xTarget ~= nil then
				TriggerEvent("atlantisNSA:billing",xPlayer.name, xTarget.name, label, amount)
			...
			if xTarget ~= nil then
				TriggerEvent("atlantisNSA:billing",xPlayer.name .. " -> ".. xTarget.name, sharedAccountName, label, amount)
				MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)',
				{
					['@identifier']  = xTarget.identifier,
					['@sender']      = xPlayer.identifier,
...
RegisterServerEvent('komornik:elo')
AddEventHandler('komornik:elo', function(steamid, society, amount)
local _source = source 
local _steamid = steamid
local Target = ESX.GetPlayerFromIdentifier(_steamid)
	TriggerEvent('esx_addonaccount:getSharedAccount', society, function(account)
		MySQL.Async.execute('DELETE from billing WHERE identifier = @identifier AND target = @target', {
				['@identifier'] = _steamid,
				['@target'] = society
			}, function(rowsChanged)
				if rowsChanged then
