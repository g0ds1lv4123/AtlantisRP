...
RegisterServerEvent('esx_property:fixServerProperties')
AddEventHandler('esx_property:fixServerProperties', function()
	local _source = source
	TriggerClientEvent("esx_property:fixProperties", _source)
...
AddEventHandler('esx_property:getItem', function(owner, type, item, count)
		...
		TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
					...
					TriggerEvent("esx:withdrawalert",xPlayer.name,inventoryItem.label,count)
				end
			else
	...
	elseif type == 'item_account' then
		...
		TriggerEvent('esx_addonaccount:getAccount', 'property_' .. item, xPlayerOwner.identifier, function(account)
			...
			if roomAccountMoney >= count then
				...
				ESX.SavePlayer(xPlayer)
				TriggerEvent("esx:withdrawalert",xPlayer.name,item,count)
			else
	...
	elseif type == 'item_weapon' then
		...
		TriggerEvent('esx_datastore:getDataStore', 'property', xPlayerOwner.identifier, function(store)
			...
			ESX.SavePlayer(xPlayer)
			TriggerEvent("esx:withdrawalert",xPlayer.name,weaponName,ammo)
		end)
	...
	if type == 'item_standard' then
		...
		if playerItemCount >= count and count > 0 then
			TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
				...
				TriggerEvent("esx:depositalert",xPlayer.name,inventory.getItem(item).label,count)
			end)
		else
	...
	elseif type == 'item_account' then
		...
		if playerAccountMoney >= count and count > 0 then
			...
			TriggerEvent('esx_addonaccount:getAccount', 'property_' .. item, xPlayerOwner.identifier, function(account)
				...
				ESX.SavePlayer(xPlayer)
				TriggerEvent("esx:depositalert",xPlayer.name,item,count)
			end)
		else
			TriggerClientEvent('esx:showNotification', _source, _U('amount_invalid'))
		end
...
function PayRent(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM owned_properties WHERE rented = 1', {}, function (result)
		for i=1, #result, 1 do
			...
			local totalPrice = result[i].price
			-- message player if connected
			if xPlayer then
				xPlayer.removeAccountMoney('bank', totalPrice)
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('paid_rent', ESX.Math.GroupDigits(totalPrice)))
