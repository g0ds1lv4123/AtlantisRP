...
function SlotMenu()
  ClearMenu()
  Menu.addTitle("Zagraj")
  Menu.addButton("~h~Kręć","Spin",nil)
...
AddEventHandler("esx_slotmachine:1", function(MoneyRecive,a,b,c)
	...
	elseif a == b or b == c then
		...
		Chat('^2WYGRANA:^0 '..a..' '..b..' '..c)
	else
		Chat('^1PRZEGRANA^0: '..a..' '..b..' '..c)
	end
end)
...
Citizen.CreateThread(function()
  for k,v in ipairs(moneymachine_slot)do
    ...
    AddTextComponentString("Jednoręki bandyta")
		...
		for k,v in ipairs(moneymachine_slot) do
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 20.0)then
				...
				if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 1.0)then
					DisplayHelpText("Wciśnij ~INPUT_CONTEXT~   ~y~by zagrać")
						if IsControlJustPressed(1,input["E"]) then
                         SlotMenu()
                         Menu.hidden = not Menu.hidden
    end
    if not Menu.hidden then
    	DrawRect(0.6,  0.45,  0.1,  0.05,  0,0,0,255)
    	drawText(1.05,  0.42+0.5, 1.0,1.0,0.6,'~g~$~w~'..tostring(money),255,0,0,255,7)
    end
    Menu.renderGUI()
  end
			end
