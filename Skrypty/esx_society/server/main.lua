...
ESX.RegisterServerCallback('esx_society:job2', function(source, cb)
	local player = getIdentity(source)
	local job2name = player.job2
		...
		MySQL.Async.fetchAll('SELECT firstname, lastname, identifier, job2, job2_grade FROM users WHERE job2 = @job2 ORDER BY job2_grade DESC', {
			...
			for i=1, #results, 1 do
				table.insert(employees, {
					...
					identifier = results[i].identifier,
					job2 = results[i].job2,
					job2_grade = results[i].job2_grade
				})
			end
...
ESX.RegisterServerCallback('esx_society:setJob2', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
		...
		if xTarget then
			...
			elseif type == 'promote' then
				TriggerClientEvent('esx:showNotification', xTarget.source, _U('you_have_been_promoted'))
				TriggerClientEvent('update:job2grade', xTarget.source)
			elseif type == 'fire' then
				TriggerClientEvent('esx:showNotification', xTarget.source, _U('you_have_been_fired', xTarget.getJob().label))
			end
