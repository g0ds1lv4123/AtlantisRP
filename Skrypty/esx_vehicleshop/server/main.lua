...
function RemoveOwnedVehicle(plate)
	MySQL.Async.execute('DELETE FROM vehicles_garage WHERE JSON_EXTRACT(vehicle, "$.Plate") = @plate', {
		['@plate'] = plate
	})
end
...
RegisterServerEvent('esx_vehicleshop:gowno')
AddEventHandler('esx_vehicleshop:gowno', function (vehicle, newPlate)
local _source = source
local xPlayer = ESX.GetPlayerFromId(_source)
local _vehicle = vehicle
local _newPlate = newPlate

TriggerEvent("kakozord:donies", xPlayer.name, " EVENT NAME: esx_vehicleshop:buyVehicle , zakup pojazdu ".._vehicle.." rejestracja ".._newPlate)
end)
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
	...
	if xPlayer.getMoney() >= vehicleData.price then
		...
		account.addMoney(vehicleData.price /2)
		end)
		cb(true)
	else
		cb(false)
...
ESX.RegisterServerCallback('esx_vehicleshop:resellVehicle', function (source, cb, plate, model)
	...
	MySQL.Async.fetchAll('SELECT * FROM rented_vehicles WHERE plate = @plate', {
		...
		if result[1] then -- is it a rented vehicle?
			...
			MySQL.Async.fetchAll('SELECT * FROM vehicles_garage WHERE owner = @owner AND JSON_EXTRACT(vehicle, "$.Plate") = @plate', {
				['@owner'] = xPlayer.identifier,
				['@plate'] = plate
			}, function (result)
