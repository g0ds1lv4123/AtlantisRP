...
if bankMenu then
	...
	if isNearATMorBank then
			DisplayHelpText("Naciśnij ~INPUT_PICKUP~ aby skorzystać z terminalu bankowego ~b~")
...
RegisterCommand("przelew", function(source, args, raw)
	SetCurrentPedWeapon(PlayerPedId(), -1569615261,true)
	Citizen.Wait(1)
	przelewtelefonprop()
	przelewkomendaanim()
    TriggerServerEvent('bank:transfer', tonumber(args[1]), tonumber(args[2]))
end, false)
